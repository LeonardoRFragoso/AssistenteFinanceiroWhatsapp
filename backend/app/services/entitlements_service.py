...
class EntitlementsService:
    ...
    def _allowed_response(self, plan: SubscriptionPlan) -> Dict[str, Any]:
        return {"allowed": True, "plan": plan.code}

    def _denied_response(
        self, plan: SubscriptionPlan, reason: str, limit: int, current_usage: int
    ) -> Dict[str, Any]:
        return {
            "allowed": False,
            "reason": reason,
    ...
    async def can_create_charge(self, organization_id: int) -> Dict[str, Any]:
        ...
        if not plan:
            return {"allowed": False, "reason": "no_plan_found", "plan": "none"}
        current = usage["charges_created"]
        limit = plan.max_charges_per_month
        if current >= limit:
            return self._denied_response(plan, "monthly_charge_limit_reached", limit, current)
        return self._allowed_response(plan)
    ...
    async def can_create_customer(self, organization_id: int) -> Dict[str, Any]:
        ...
        if current >= limit:
            return self._denied_response(plan, "customer_limit_reached", limit, current)
        return self._allowed_response(plan)
    ...
    async def can_create_template(self, organization_id: int) -> Dict[str, Any]:
        ...
        if current >= limit:
            return self._denied_response(plan, "template_limit_reached", limit, current)
        return self._allowed_response(plan)
    ...
    async def can_create_recurring_task(self, organization_id: int) -> Dict[str, Any]:
        ...
        if current >= limit:
            return self._denied_response(plan, "recurring_task_limit_reached", limit, current)
        return self._allowed_response(plan)
    ...
    async def can_use_collection_rules(self, organization_id: int) -> Dict[str, Any]:
        ...
        if not plan.allow_collection_rules:
            return {
                "allowed": False,
                "reason": "collection_rules_not_included",
                "plan": plan.code,
            }
        return self._allowed_response(plan)
    ...
    async def can_add_team_member(self, organization_id: int) -> Dict[str, Any]:
        ...
        if current_members >= limit:
            return self._denied_response(plan, "team_member_limit_reached", limit, current_members)
        return self._allowed_response(plan)
    ...
    async def can_process_whatsapp_message(self, organization_id: int) -> Dict[str, Any]:
        ...
        if limit is not None and current >= limit:
            return self._denied_response(plan, "whatsapp_message_limit_reached", limit, current)
        return self._allowed_response(plan)
